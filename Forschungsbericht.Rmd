...
Das Hauptnetzwerk besteht aus XX Komponenten mit XX und XX Knoten, die nicht miteinander verbunden sind. Die Dichte im Netzwerk beträgt XX Prozent von allen möglichen Verbindungen. Die maximale Pfaddistanz beträgt XX Schritte.
...
plot(space,
     ...
     edge.arrow.size=.5,
     vertex.size=10,
     main ="Übersicht des Gesamtnetzwerk SpaceX")
```
...
space <- delete_edge_attr(space, "position")
space <- delete_edge_attr(space, "time")
list.edge.attributes(space)
...
is_simple(space)
space_sim <- simplify(space,edge.attr.comb=list(weight="sum"))
is_simple(space_sim)
space_sim

# die aggregierten Werte für weight von s_sim werden deutlich.
list.edge.attributes(space_sim)
edge.attributes(space_sim)$weight
...
plot(space_sim,
     ...
     vertex.size=8,
     edge.curved=.2,
     edge.curved=curve_multiple(space_sim),
     main="Gesamtnetzwerk vereinfacht",
     sub="Aggregierte Gewichte von Position und Zeit im Unternehmen")
```
...
# vertex.attributes(space)
list.edge.attributes(space)
# edge.attributes(space)
# edge.attributes(space)$weight
```
